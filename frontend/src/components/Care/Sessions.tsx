...
import { Card, CardContent, ConfirmModal, Loading } from "../../components";
...
const Sessions = () => {
  ...
  const [, setSelectedCare] = useState<HelpSessionProps | null>(null);
  ...
  const [itemToAction, setItemToAction] = useState<HelpSessionProps | null>(
    null
  );
  ...
  const openModal = (e: HelpSessionProps | null = null): void => {
    setSelectedCare(e);
    setIsModalOpen(true);
  };
  ...
  const markAsCompleted = async (sessionId: string) => {
    try {
      const response = await refreshUser(`${SESSION_API_URL}/${sessionId}`, {
        method: "PUT",
        headers: {
      ...
      setSuccessMessage(t("dashboard.sessionCompleted"));
  ...
  const getConfirmMsg = (action: string) => {
    if (action === "completed") {
      setConfirmMessage(t("confirm.completedSession"));
    }
    if (action === "cancelled") {
      setConfirmMessage(t("confirm.cancelledSession"));
    }
  };
                        ...
                        <div className="relative">
                          ...
                          <span
                            className={`absolute top-2 right-2 z-20 px-2 py-1 bg-${
                              getFarbe(session?.status).color
                            }-600 text-white text-xs font-bold rounded shadow-md`}
                          >
                            {getFarbe(session?.status).icon}{" "}
                            {getFarbe(session?.status).text}
                          </span>
                            ...
                            <CardContent className="px-4 md:px-6">
                                ...
                                <div className="flex-1 w-full">
                                  ...
                                  {session.status !== "active" && (
                                    <div className="grid grid-cols-1 md:grid-cols-1 gap-3 mb-4">
                                      ...
                                      <div className="flex items-center gap-2">
                                        <div className="p-2 bg-blue-100 rounded">
                                          <CircleHelp className="w-5 h-5 text-orange-600" />
                                        </div>
                                        <div>
                                          <p className="text-sm text-gray-500">
                                            {session.final}
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <div>
                                        ...
                                        <p className="text-sm font-semibold text-gray-900 truncate">
                                          {session.requestId
                                            ? typeof session.userRequesterId ===
                                                "object" &&
                                              session.userRequesterId
                                                ...
                                                session.userRequesterId
                                                  ?.lastName
                                            : typeof session.offerId?.userId ===
                                                "object" &&
                                              session.offerId?.userId
                                                ?.firstName +
                                                " " +
                                                session.offerId?.userId
                                                  ?.lastName}
                              ...
                              {session.status === "active" && (
                                <div className="flex gap-2 mt-5 pt-2 border-t">
                                  <button
                                    onClick={() => {
                                      setItemToAction(session);
                                      getConfirmMsg("completed");
                                      setShowConfirm(true);
                                    }}
                                    className="flex-1 flex items-center cursor-pointer justify-center gap-1 px-3 py-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100 transition-all text-sm font-semibold"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="flex-1 flex items-center cursor-pointer justify-center gap-1 px-3 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-all text-sm font-semibold"
                                  >
                                    <XCircle size={16} />
                          ...
                          {showConfirm && (
                            <ConfirmModal
                              ...
                              onConfirm={() => {
                                markAsCompleted(itemToAction!._id);
                              }}
