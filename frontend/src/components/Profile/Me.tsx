...
import { AddressInput, Card, CardContent, Loading } from "..";
import { AnimatePresence, motion } from "framer-motion";
import { Mail, Phone, MapPin, Star, Edit2, Save, X, Award } from "lucide-react";
import { useNavigate } from "react-router";

const Me = () => {
  const { user, fetchUser, refreshUser, issuesToFieldErrors, loading } =
    useAuth();
  const { t } = useLanguage();
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FormData.append expects a string or Blob — convert numbers to strings
      ...
      setInfoMsg(t("profile.updateSuccess"));
      window.scrollTo(0, 0);
      setEditing(false);
    } catch (error: any) {
        ...
        >
          <Card className="rounded-xl shadow-lg border-0 overflow-hidden">
            <CardContent className="p-8">
              ...
              {editing && (
                <>
                  <label className="flex items-center gap-2 cursor-pointer text-blue-600 font-medium text-sm">
                    <input
                      type="file"
                      accept="image/*"
                      onChange={(e) => {
                      ...
                      className="hidden"
                    />
                    <span className="w-md bg-gradient-to-r from-blue-600 to-blue-700 text-white text-center py-3 rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed font-bold text-lg shadow-lg hover:shadow-xl hover:scale-105 duration-300 cursor-pointer">
                      Upload Image
                    </span>
                  </label>
                  {fieldErrors.image && (
                  ...
                  {formData.image && (
                    <img
                      ...
                      alt="Preview"
                      className="w-32 h-32 object-cover rounded mt-2 border border-gray-300"
                    />
                  )}
                </>
              )}
              ...
              {editing && (
                <button
                  ...
                  disabled={saving}
                  className="w-full cursor-pointer mt-6 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white py-3 rounded-lg transition-all font-semibold"
                >
                  <Save size={18} />
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="cursor-pointer text-blue-600 hover:text-blue-700 font-semibold text-sm"
                >
                  View All →
                </button>
              </div>
